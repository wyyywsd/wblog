...
function show_user_article_by_page(page) {
    $.ajax({
        url: "/show_user_articles/" + page, success: function (result) {
            $("#div1").html(result);
        }
    });
}
...
$(document).ready(function () {
    ...
    if (pageCount > 1) {
        $("#user_article_page").html(function () {
                var a_h = ""
                //上一页
                if (currentPage == 1) {
                    a_h = "<li  class=\"previous disabled\"><span><span aria-hidden=\"true\">&laquo;</span></span></li>"
                } else {
                    a_h = "<li><a onclick=\"show_user_article_by_page(" + (currentPage - 1) + ")\" style=\"cursor:pointer;\" aria-label=\"Previous\"><span aria-hidden=\"true\">&laquo;</span></a></li>"
                }
                for (var i = 0; i < pageCount; i++) {
                    var c = i + 1
                    var a_active = "<li class = \"active\"><a class =\"user_article_page_a\" style=\"cursor:pointer;\" onclick=\"show_user_article_by_page(" + c + ")\">" + c + "</a></li>"
                    var a_previous = "<li class=\"previous disabled\"><span>...</span></li>"
                    var a_normal = "<li><a class =\"user_article_page_a\" style=\"cursor:pointer;\" onclick=\"show_user_article_by_page(" + c + ")\">" + c + "</a></li>"
                ...
                if (currentPage == pageCount) {
                    ...
                    a_h = a_h + "<li><a onclick=\"show_user_article_by_page(" + (currentPage + 1) + ")\" style=\"cursor:pointer;\" aria-label=\"Next\"><span aria-hidden=\"true\">&raquo;</span></a></li>"
                }
                return a_h
            }
        );
